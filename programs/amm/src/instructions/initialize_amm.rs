...
use crate::{errors::AMMErrors, Admin, AmmInitalized, InitTokenParams, AMM};
...
pub struct InitalizeAmm<'info> {
    ...
    #[account(
        init,
        seeds = [b"mint", signer.key().as_ref(), params.seed.to_le_bytes().as_ref()],
        bump,
        payer = signer,
    ...
    #[account(
        init,
        payer = signer,
        seeds = [b"amm", mint.key().as_ref(), params.seed.to_le_bytes().as_ref()],
...
impl<'info> InitalizeAmm<'info> {
    pub fn create_mint(
        ...
        let binding = metadata.seed.to_le_bytes();
        let seeds = &[
            "mint".as_bytes(),
    ...
    pub fn mint_to_reserve(
        ...
        let amount = metadata.mint_cap * u64::pow(10, self.mint.decimals as u32);
        mint_to(ctx, amount)?;
        Ok(())
    }
